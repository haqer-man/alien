...
def play_video(starting_position: float, duration: float):
    global player

    print("[i] Made it to play_video function.")
    if not player.is_playing():
        print("[i] Player is not currently playing. Continuing.")
        # player = vlc.MediaPlayer("file://{}".format(fi))
        player.audio_set_volume(0)
        player.set_position(starting_position)
...
def main():
    ...
    while True:
        ...
        player = vlc.MediaPlayer("file://{}".format(fi))
        print("[i] Initialized MediaPlayer.")

        sleep(2)
        print("[i] Slept for 2.")

        total_duration = player.get_length()
        start_time = randint(0, total_duration - 10)
        start_position = start_time / total_duration
        played_duration = randint(5, total_duration - start_time)


        play_video(start_position, played_duration)

        sleep(1)
        
        print("Playing {} (total of {} seconds). starting at {}. playing for {}"\
        .format(fi, total_duration, start_time, played_duration))
        sleep(played_duration / 1000 - 2)
